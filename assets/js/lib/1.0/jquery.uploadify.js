		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					button_placeholder: this,				//如果id找不到，则直接使用dom本身
					buttonClass     : 'btn-primary',                 // A class name to add to the browse button DOM object
					...
					buttonText      : $this.text() || '上传文件',     	// The text to use for the browse button
					checkExisting   : false,              // The path to a server-side script that checks for existing files on the server
					debug           : false,              // Turn on swfUpload debugging mode
					fileObjName     : 'Filedata',         // The name of the file object to use in your server-side script
					fileSizeLimit   : '2048 MB',                  // The maximum size of an uploadable file in KB (Accepts units B KB MB GB if string, 0 for no limit)
					...
					overrideTemplate: false,              // The template for the file item in the queue
					// Events
				...
				if (flashInstalled) {
					...
					var $button = $('<button />', {
						...
						'class' : 'btn ' + settings.buttonClass
					});
	...
	var handlers = {
		...
		onSelect : function(file) {
			...
			if (!settings.overrideTemplate) {
				...
				$('#' + settings.queueID).append(itemTemplate);
			}
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						alert("You have attempted to queue too many files.\n" + (errorMsg === 0 ? "You have reached the upload limit." : "You may select " + (errorMsg > 1 ? "up to " + errorMsg + " files." : "one file.")));
		...
		onUploadError : function(file, errorCode, errorMsg) {
			...
			switch(errorCode) {
				...
				case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:
					...
					break;
				default:
					errorString = "未知异常,ErrorCode:" + errorCode;
			}
			...
			if ($.inArray('onUploadError', settings.overrideEvents) < 0) {
				...
				if (errorCode != SWFUpload.UPLOAD_ERROR.SPECIFIED_FILE_ID_NOT_FOUND && file.status != SWFUpload.FILE_STATUS.COMPLETE && errorString) {
					alert(errorString);
				}
			}
		...
		onUploadSuccess : function(file, data, response) {
			...
			// 默认模板处理
			if (!settings.overrideTemplate) {
				var html = '<div id="'+file.id+'" class="finish-queue-item" data-fileid="' + data.fileid + '">' +
                    Util.GetFileImgIcon(file.type) +
