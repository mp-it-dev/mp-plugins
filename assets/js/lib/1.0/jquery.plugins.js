...
;(function($, global) {
    ...
    Table.prototype.getPageData = function () {
        ...
        $pager.pager({
            ...
            pageSizeArray   : options.pageSizeArray,
            pageLength      : options.pageLength,
            pageInfo        : options.pageInfo,
            skipPage        : options.skipPage,
            localPage       : options.localPage,
            ...
            success         : function (res, reloadFlag) {
                if (options.localPage) {
                    options.dataList = res;
                } else {
                    options.dataList = res[options.dataField];
                }
    ...
    Table.prototype.initTable = function (reloadFlag) {
        ...
        setTimeout(function () {
            var $thead = $container.find('.table-head'),
                ...
                tpageHeight = $container.find('.table-pager').outerHeight(),
                sWidth = PluginDep.scrollBarWidth();

            //计算最大高度
            if (options.maxHeight) {
                $tbody.css('max-height', options.maxHeight - theadHeight - tpageHeight);
            }

            //计算高度
            if (options.height) {
                $tbody.css('height', options.height - theadHeight - tpageHeight);
            }
            ...
            for (var i = 0, l = $thead_ths.length; i < l; i++) {
                w = parseInt($thead_ths[i].style.width) || $thead_ths.eq(i).width();
                totalW += w;
    ...
    Table.prototype.initTbody = function () {
        ...
        if (dataList == null || dataList.length == 0) {
            $tbody.append('<tr class="table-tr table-noresult"><td colspan="'+colLen+'" align="center">无结果</td></tr>');
    ...
    function bindCommonEvents () {
        ...
        //解决window.resize再回到原来尺寸时table-body可能出现滚动条的问题
...
(function($, window, undefined) {
    ...
    var methods = {
        ...
        getPageIndex: function () {
            return this.data(namespace).options.pageIndex;
        },
    ...
    Pager.prototype.requestData = function (pageIndex, reloadFlag) {
        ...
        var ajaxOption = {
            ...
            success: function (res) {
                ...
                options.total = options.localPage ? res[options.dataField].length : res[options.totalField];
                ...
                if (options.localPage) {
                    options.localData = res[options.dataField] || [];
                    options.success(options.localData.slice(0, options.pageIndex * options.pageSize));
                } else {
    ...
    Pager.prototype.skipPage = function (pageIndex) {
        ...
        this.initPage();
        options.success(options.localData.slice((pageIndex - 1) * options.pageSize, pageIndex * options.pageSize), true);
    }
...
(function($, window, undefined) {
    ...
    UiSelect.prototype.init = function () {
        ...
        settings.width = Math.max(this.ele.outerWidth(), 70);
    ...
    UiSelect.prototype.initDom = function () {
        ...
        this.target = $target;
        this.target.outerWidth(settings.width);
        this.createList();
        this.bindEvents();
    }
            ...
            for (var i in data) {
                if (data[i].children) {
                    ...
                    for (var j in children) {
                        ...
                        $('<li class="ui-select-li" data-value="'+d.value+'">'+icon+'<div class="ui-select-content">'+d.text+'</div></li>').appendTo($list);
                    }
                } else {
                    var d = data[i];
                    ...
                    $('<li class="ui-select-li" data-value="'+d.value+'">'+icon+'<div class="ui-select-content">'+text+'</li>').appendTo($list);
                }
            }
        }
...
;(function ($) {
    ...
    Gallery.prototype.cacheImg = function () {
        ...
        $ul.width(data.length * $li.outerWidth());
    ...
    Gallery.prototype.centerImgbox = function () {
        ...
        //设置行高
        var h = $(top).height() - $ele.find('.gallery-toolbar').outerHeight();

        if ($ele.find('.gallery-showList').hasClass('up')) {
            h -= $ele.find('.gallery-imgList').outerHeight();
        }
...
;(function () {
    ...
    UiResize.prototype.init = function () {
        ...
        if (settings.vertical) {
            left = this.container.outerWidth() - settings.border - this.container.getCss('border-right');
            ...
            top = this.container.outerHeight() - settings.border - this.container.getCss('border-bottom');
